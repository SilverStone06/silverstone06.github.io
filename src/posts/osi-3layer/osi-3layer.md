...
앞서 데이터링크 계층에서는 **같은 네트워크 안에서 안정적으로 프레임을 주고받는 역할**에 대해 정리했습니다. 하지만 네트워크는 하나의 구역(LAN)만 있는 것이 아니라, 수많은 네트워크가 서로 연결되어 데이터를 주고받습니다. 

이제 그 범위를 확장해, **서로 다른 네트워크 간의 통신**을 담당하는 **네트워크 계층(Network Layer)** 을 정리해 봅시다.
...
| 구분 | 설명 |
| --- | --- |
| **주소 크기** | 4 Byte (IPv4 기준) |
| **기능** | IP 주소 지정, 단편화 및 재조합 |
| **전송 단위** | 패킷(Packet) |
...
| 필드 | 역할 |
| --- | --- |
| **식별자(Identifier)** | 분할된 패킷을 다시 조립할 때 구분자 역할 |
| **플래그(Flag)** | 3비트 구성 — `DF`(단편화 금지), `MF`(추가 단편화 여부) |
| **단편화 오프셋(Fragment Offset)** | 각 조각의 순서 지정 |
...
**ARP**는 동일 네트워크 내에서 **IP 주소를 MAC 주소로 변환**하는 프로토콜입니다.
...
네트워크 계층의 가장 기본이자 중요한 요소는 **IP 주소**입니다.

| 구분 | 설명 |
| --- | --- |
| **공인 IP (Public IP)** | 전 세계에서 유일하게 할당받은 주소. 인터넷 통신에 사용 |
| **사설 IP (Private IP)** | 내부 네트워크에서만 사용. 외부 공개 불가 |
| **정적 IP (Static)** | 수동 설정으로 고정된 IP 주소 |
| **동적 IP (Dynamic)** | DHCP 서버를 통해 자동 할당되는 IP 주소 |

DHCP(Dynamic Host Configuration Protocol)는 자동으로 IP 주소를 할당해 **관리 효율을 높이고, IP 자원을 절약**합니다.

---

## 4️⃣ 라우팅(Routing)

라우팅은 패킷이 목적지까지 가는 **최적의 경로를 결정하는 과정**입니다.
...
**Hop**은 패킷이 하나의 장치를 통과하는 과정을 뜻합니다.
...
**최적의 경로를 자동으로 찾아내는 기술**입니다.

크게 **AS(Autonomous System, 자율 시스템)** 내부와 외부 통신용으로 나뉩니다.

| 구분 | 종류 | 설명 |
| --- | --- | --- |
| **IGP (Interior Gateway Protocol)** | RIP, OSPF | 동일한 AS 내부에서 라우터 간 경로 정보를 교환하는 프로토콜 |
| **EGP (Exterior Gateway Protocol)** | BGP | 서로 다른 AS 간의 경로 정보를 교환하는 프로토콜 |
...
링크 상태(link-state) 기반 프로토콜로, **Dijkstra 최단 경로 알고리즘**을 이용해 네트워크 전체 구조를 파악하고 최적의 경로를 계산합니다. 중·대규모 네트워크 환경에서 가장 널리 사용됩니다.
...
**IGP**는 내부 네트워크의 길잡이, **EGP**는 네트워크 간의 외교관 같은 역할을 합니다.

---

### 🔹 라우팅 테이블(Routing Table)

라우터는 **라우팅 테이블**을 참조해 패킷을 어디로 보낼지 결정합니다.

| 항목 | 설명 |
| --- | --- |
| **Next Hop (게이트웨이)** | 다음 라우터의 주소 |
| **Interface** | 패킷을 내보낼 네트워크 인터페이스 |
| **Metric** | 비용(거리·속도 등)에 기반한 경로 우선순위 |
...
**최적 경로를 자동으로 찾아내는 기술**입니다.

| 구분 | 종류 | 설명 |
| --- | --- | --- |
| **IGP (Interior Gateway Protocol)** | RIP(거리 벡터), OSPF(링크 상태) | 동일 AS 내부 통신용 |
| **EGP (Exterior Gateway Protocol)** | BGP | AS 간 통신 시 사용 |

---

## 5️⃣ 마무리 — 네트워크 간의 길을 만드는 계층

네트워크 계층은 **패킷이 목적지까지 도달할 수 있도록 길을 안내하는 계층**입니다.
